...
public class ProfileManager : MonoBehaviour
{
    ...
    public void DeleteProfile(string guid)
    {
        ...
        // 2. DELETE PROFILE FOLDER AND ALL ITS CONTENT
        ...
        // 3. DELETE SAVE DATA (YOUR SAVE SYSTEM LOGIC)
        SaveSystem.DeleteSave(guid);
        Debug.Log($"[ProfileManager] Deleted save data for: {guid}");

        // 4. REMOVE FROM PROFILE INDEX
        profileHeaders.RemoveAll(h => h.id == guid);
        SaveProfileIndex();

        // 5. CLEAR CURRENT PROFILE IN MEMORY IF MATCHING
