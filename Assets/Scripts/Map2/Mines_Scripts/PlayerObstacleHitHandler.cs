...
public class PlayerObstacleHitHandler : MonoBehaviour
{
    ...
    private ScreenFeedback feedback; // your existing UI flash script
    ...
    public void OnObstacleHit(ObstacleHit obstacle)
    {
        ...
        // Slowdown - uses existing logic correctly
        if (followRoad != null)
            followRoad.OnMissedJellyfish();

        // UI pulse (same as red jellyfish)
        if (feedback != null)
